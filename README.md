...
**Fund your wallet before deploying.** Deployment requires your wallet to hold tDUST for transaction fees. Request funds from the local dev network's faucet before running `yarn deploy` — deployment will fail with an insufficient funds error if the wallet is unfunded.
